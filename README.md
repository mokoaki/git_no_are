...
**git add**  
「指定されたファイルをステージング領域へ追加するよ！」  

ステージング領域とは、この後で出てくるコミットの候補として登録する場所みたいなものです  
ステージング領域はインデックス領域、またはステージング、インデックス とも言います  
ぜんぶ同じような意味で覚えておいていいです  
新規のファイルでも、既に管理されているファイルでも、あまり気にすることなく「Gitに登録する準備をする」ような感じでいいです
...
> git add app/
# カレントディレクトリから見て相対パスにある、 app/ 配下のステージング可能なファイルを全て追加します
# 可能なファイルとは、新規追加ファイルとGitが管理しているファイルの内、修正済みのファイルです。
# 逆に、前回のコミットから何も変更されていないファイルは add できない、ということですね
...
**git commit**  
「ステージングされたファイルを歴史に記録するよ！たとえ君が忘れたとしても、永遠に！」  
...
```
> git commit
# ステージング済みのファイルをコミットします。突然エディタが起動しますが慌てないで下さい。コミットメッセージを入力する画面です、普通に入力保存終了して下さい
...
**git checkout**  
「その修正はちょっとダサいな。もう一度、コミットしておいた時点からやりなおしてみないか？」  
...
#そろそろ、ややこしくなってきます
一人でバージョン管理するだけなら、これまでのコマンドでなんとかならなくもありません  
しかし、他人との連携、複数作業同時展開、歴史の改変　等、もう少し手の込んだ作業を行おうと思ったら  
